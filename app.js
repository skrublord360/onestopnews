      ...
      if (!subcategories.length) {
        return `
          <div class="topic-item">
            <button class="topic-button direct-topic ${state.selectedCategory === category.id ? "active" : ""}" type="button" data-direct-category="${category.id}" data-direct-subcategory="${escapeHtml(category.subcategories[0] || "")}">
...
document.addEventListener("click", async (event) => {
  const directButton = event.target.closest("[data-direct-category]");
  ...
  if (directButton) {
    state.selectedCategory = directButton.dataset.directCategory;
    state.selectedSubcategory = directButton.dataset.directSubcategory;
    state.openMenuId = "";
    await loadArticles();
  }
