...
function articleCount(categoryId, subcategory = "") {
  return state.articles.filter((article) => {
    if (categoryId !== "top" && article.category !== categoryId) return false;
    if (subcategory && !subcategory.startsWith("All ") && article.subcategory !== subcategory) return false;
    return true;
  }).length;
}
...
function renderFeed() {
  ...
  els.resultCount.textContent = `${state.articles.length} article${state.articles.length === 1 ? "" : "s"} shown`;
...
function renderStatus(payload) {
  ...
  els.statusPills.innerHTML = `
    ...
    <span>${state.articles.filter((article) => article.summary).length} summarized</span>
  `;
}
